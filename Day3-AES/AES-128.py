...
# -*- coding: utf-8 -*-
# author： snow
# datetime： 2021/10/27 15:36
# ide： PyCharm


nb = 4  # number of coloumn of State (for AES = 4)
nr = 10  # number of rounds ib ciper cycle (if nb = 4 nr = 10)  ciper循环的轮数
...
def shift_rows(state, inv=False):
    ...
    touch the first row. When encrypting transformation uses left shift, in decryption - right shift
...
def mix_columns(state, inv=False):
    ...
    Detailed information in AES standard.
...
def add_round_key(state, key_schedule, round=0):
    ...
    这种转换将State和KeySchedule结合在一起。Xor
    State和RoundSchedule(KeySchedule的一部分)。
    """
